...
- За время использования системы в проекте Project 2.0 накопилось 282 записи в `entries`. По факту это шум: задачи (`tasks`), отчёты прогресса (`progress`), эпизодические баги (`issues`) — то, что нативно живёт в Azure DevOps work items, в git/PR-истории и в импортированных сессиях соответственно.
...
- Cleanup существующих 282 шумных записей в Project 2.0 — отдельная задача после прогона нового extractor 1–2 недели на новых сессиях. Старые записи под decay уйдут сами.
...
```json
{
  "azure_work_items": ["WORK-1001", "WORK-1002"],
  "azure_prs": ["PR-1234"],
...
- Все 282 записи Project (и другие проекты) сохраняются. Новые поля заполняются дефолтами:
  - `auto_generated = false`
  - `confirmation_count = 1`
...
- Прогон 1 реальной сессии Project через ручной reimport → проверка кандидатов в Web UI
