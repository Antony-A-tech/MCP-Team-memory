...
- За время использования системы в реальном проекте накопилось 282 записи в `entries`. По факту это шум: задачи (`tasks`), отчёты прогресса (`progress`), эпизодические баги (`issues`) — то, что нативно живёт в Azure DevOps work items, в git/PR-истории и в импортированных сессиях соответственно.
...
- Cleanup существующих 282 шумных записей — отдельная задача после прогона нового extractor 1–2 недели на новых сессиях. Старые записи под decay уйдут сами.
...
- Все 282 существующие записи (по всем проектам) сохраняются. Новые поля заполняются дефолтами:
  - `auto_generated = false`
  - `confirmation_count = 1`
...
- Прогон 1 реальной сессии через ручной reimport → проверка кандидатов в Web UI
